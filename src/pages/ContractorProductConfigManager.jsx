  ...
  return (
    <div className="p-3 sm:p-6">
      ...
      >
        ...
        {/* Labor Rates Tab */}
        <TabPane tab="Labor Rates" key="rates">
          <div className="space-y-6">
            <Alert
              message="Labor Rates Configuration"
              description="Configure default labor rates for interior and exterior work. These defaults will be applied to new product configurations."
              type="info"
              showIcon
              className="mb-4"
            />

            <Collapse defaultActiveKey={['interior', 'exterior']} className="bg-white">
              {/* Interior Labor Rates */}
              <Collapse.Panel header="Interior Labor Rates" key="interior">
                <div className="mb-3">
                  <Button
                    type="primary"
                    icon={<PlusOutlined />}
                    onClick={() => handleCreateLaborRate('interior')}
                    size="small"
                    disabled={savingLaborRate || deletingLaborRate !== null}
                  >
                    Add Interior Rate
                  </Button>
                </div>
                
                {laborDefaults?.laborRates?.interior && laborDefaults.laborRates.interior.length > 0 ? (
                  <Table
                    dataSource={laborDefaults.laborRates.interior.map((rate, index) => ({ ...rate, key: index }))}
                    pagination={false}
                    size="small"
                    columns={[
                      {
                      ...
                      },
                      {
                        ...
                        dataIndex: 'rate',
                        key: 'rate',
                        render: (rate, record) => `$${Number(rate).toFixed(2)}/${record.unit}`,
                      },
                      {
                        title: 'Description',
                        ...
                        render: (_, record, index) => (
                          <Space size="small">
                            <Button
                              size="small"
                              icon={<EditOutlined />}
                              onClick={() => handleEditLaborRate(record, 'interior', index)}
                              disabled={savingLaborRate || deletingLaborRate !== null}
                            >
                              Edit
                            </Button>
                            <Popconfirm
                              title="Delete this labor rate?"
                              onConfirm={() => handleDeleteLaborRate('interior', index)}
                              okText="Yes"
                              cancelText="No"
                            >
                              <Button 
                                ...
                                danger 
                                icon={<DeleteOutlined />}
                                loading={deletingLaborRate === `interior-${index}`}
                                disabled={deletingLaborRate !== null || savingLaborRate}
                              >
                                Delete
                              </Button>
                            </Popconfirm>
                  ...
                  <div className="text-center text-gray-400 py-8">
                    <p>No interior labor rates configured</p>
                    <p className="text-sm">Click "Add Interior Rate" to get started</p>
                  </div>
                )}
              </Collapse.Panel>

              {/* Exterior Labor Rates */}
              <Collapse.Panel header="Exterior Labor Rates" key="exterior">
                <div className="mb-3">
                  <Button
                    type="primary"
                    icon={<PlusOutlined />}
                    onClick={() => handleCreateLaborRate('exterior')}
                    size="small"
                    disabled={savingLaborRate || deletingLaborRate !== null}
                  >
                    Add Exterior Rate
                ...
                {laborDefaults?.laborRates?.exterior && laborDefaults.laborRates.exterior.length > 0 ? (
                  <Table
                    dataSource={laborDefaults.laborRates.exterior.map((rate, index) => ({ ...rate, key: index }))}
                    pagination={false}
                    size="small"
                    columns={[
                      {
                        ...
                        render: (_, record, index) => (
                          <Space size="small">
                            <Button
                              size="small"
                              icon={<EditOutlined />}
                              onClick={() => handleEditLaborRate(record, 'exterior', index)}
                              disabled={savingLaborRate || deletingLaborRate !== null}
                            >
                              Edit
                            </Button>
                            <Popconfirm
                              title="Delete this labor rate?"
                              onConfirm={() => handleDeleteLaborRate('exterior', index)}
                              okText="Yes"
                              cancelText="No"
                            >
                              <Button 
                                ...
                                danger 
                                icon={<DeleteOutlined />}
                                loading={deletingLaborRate === `exterior-${index}`}
                                disabled={deletingLaborRate !== null || savingLaborRate}
                              >
                                Delete
                              </Button>
                  ...
                  <div className="text-center text-gray-400 py-8">
                    <p>No exterior labor rates configured</p>
                    <p className="text-sm">Click "Add Exterior Rate" to get started</p>
                  </div>
                )}
              </Collapse.Panel>
            </Collapse>
        ...
        {/* Markup & Tax Tab */}
        <TabPane tab="Markup & Tax" key="markup">
          <div className="space-y-6">
            <Alert
              message="Markup & Tax Configuration"
              description="Configure default markup percentages and tax rates. These defaults will be applied to new product configurations."
              type="info"
              showIcon
              className="mb-4"
            />

            <Form form={markupForm} layout="vertical" className="max-w-2xl">
              <Card title="Default Markup & Tax Settings" className="shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Form.Item
                    name="defaultMarkup"
                    label="Default Markup Percentage"
                    rules={[
                      { required: true, message: 'Please enter default markup' },
                      { type: 'number', min: 0, max: 100, message: 'Must be between 0 and 100' },
                    ]}
                    tooltip="This markup will be applied to all product costs by default"
                  >
                    <InputNumber
                      min={0}
                      max={100}
                  ...
                  <Form.Item
                    ...
                    tooltip="Tax rate applied to material costs (labor is typically not taxed)"
                  >
                    <InputNumber
                      min={0}
                      max={100}
                ...
                <Alert
                  message="How Markup Works"
                  description={
                    <div className="text-sm space-y-2">
                      <p><strong>Default Markup:</strong> Applied to product costs when creating quotes. For example, a 15% markup on a $100 product makes it $115.</p>
                      <p><strong>Material Tax:</strong> Applied to the final material cost (after markup). Labor costs are typically not taxed.</p>
                      <p><strong>Example Calculation:</strong> Product Cost: $100 → After 15% markup: $115 → After 7% tax: $123.05</p>
                    </div>
                  }
                  type="info"
                  showIcon
                ...
                <div className="flex justify-end">
                  <Button
                    type="primary"
                    onClick={handleSaveMarkupAndTax}
                    loading={savingSettings}
                    disabled={savingSettings}
                    size="large"
                    icon={<EditOutlined />}
                  >
                    Save Markup & Tax Settings
                  </Button>
                </div>
              </Card>
            </Form>
