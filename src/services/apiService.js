...
class ApiService {
    ...
    async handleResponse(response, retryCount = 0) {
        // Record response time if start time was attached to the response object by the caller
        // (Since fetch is async, we'll just log generic response info here for now)
        const data = await response.json()

        // Log response info for debugging
        if (import.meta.env.DEV) {
            console.log(`[API] ${response.method || 'GET'} ${response.url.replace(this.baseURL, '')} - ${response.status}`);
        }
    ...
    // Admin Brands
    async getAdminBrands(params = {}, options = {}) {
        const query = new URLSearchParams(params).toString();
        return this.get(`/admin/brands${query ? `?${query}` : ''}`, null, options);
    }
    ...
    // Pricing Schemes
    async getPricingSchemes(params = {}, options = {}) {
        const query = new URLSearchParams(params).toString();
        return this.get(`/pricing-schemes${query ? `?${query}` : ''}`, null, options);
    }
     ...
     */
    async getProductConfigDefaults(options = {}) {
        return this.get('/contractor/product-configs/defaults', null, options);
    }
