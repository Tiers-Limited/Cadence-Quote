...
const mobileVerifySignup = async (req, res) => {
    ...
    try {
        ...
        const token = generateToken(userWithTenant, userWithTenant.tenant);
        const refreshToken = generateRefreshToken(userWithTenant, userWithTenant.tenant);
...
const mobileSignin = async (req, res) => {
    try {
        ...
        const refreshToken = generateRefreshToken(user, user.tenant);
...
const mobileGoogleSignIn = async (req, res) => {
    ...
    try {
        ...
        // Generate enriched tokens
        // For new users, we use bobbyTenant since we just created/fetched it
        const token = generateToken(user, user.tenant || bobbyTenant);
        const refreshToken = generateRefreshToken(user, user.tenant || bobbyTenant);
