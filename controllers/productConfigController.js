...
// Cache removed for optimization
 ...
 * OPTIMIZED: Database-side filtering and pagination, removed cache
 */
const getAllProductConfigs = async (req, res) => {
    try {
        ...
        if (search) {
            ...
            const escapedSearch = sequelize.escape(searchLower);

            where[Op.or] = [
                // Search in Global Product (via association)
                { '$globalProduct.name$': { [Op.iLike]: searchLower } },
                { '$globalProduct.brand.name$': { [Op.iLike]: searchLower } },
                // Search in Custom Product JSON (simple text search on the JSON column/string)
                // This is a safe fallback ensuring we find the text anywhere in the custom definition
                // Using sequelize.escape to prevent SQL injection in the literal query
                Sequelize.literal(`CAST("ProductConfig"."custom_product" AS TEXT) ILIKE ${escapedSearch}`)
            ];
        }

        if (brandId) {
            // Filter by brand ID (only applies to Global Products usually)
            where['$globalProduct.brand_id$'] = brandId;
        }

        if (jobType) {
            const category = jobType.toLowerCase() === 'interior' ? 'Interior' : 'Exterior';
            // Search in Global Product Category OR Custom Product Category
            where[Op.and] = [
                {
                    [Op.or]: [
                        { '$globalProduct.category$': category },
                        // Safe since category is strictly controlled above via ternary
                        Sequelize.literal(`"ProductConfig"."custom_product"->>'category' = '${category}'`)
                    ]
                }
            ];
        }

        // Execute optimized DB query with pagination
        const { count, rows: configs } = await ProductConfig.findAndCountAll({
            where,
            ...
            include: [
                {
                    model: GlobalProduct,
                    as: 'globalProduct',
                    required: false, // Left join to include custom products
                    attributes: ['id', 'name', 'category', 'brandId'],
                    include: [
                        {
                            model: Brand,
                            as: 'brand',
            ...
            limit: limitNum,
            offset,
            subQuery: false, // Essential for filtering on included tables with limits
            distinct: true // Correct count for includes
        });
 ...
 * Get default values for product configurations
 * GET /api/v1/contractor/product-configs/defaults
 * OPTIMIZED: Uses raw query for performance, removed cache
 */
const getDefaults = async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        console.time('DB Query: getDefaults');
        // Get contractor settings for this tenant (raw query)
        ...
        console.timeEnd('DB Query: getDefaults');

        // If no settings exist, create default settings
        if (!settings) {
            const newSettings = await ContractorSettings.create({ tenantId });
            // Since raw: true was used above, and create returns an instance, 
            // we should technically use the instance to populate values or just use defaults.
            // For simplicity and performance, we'll just fall back to defaults in the response object below.
            settings = {};
        }
