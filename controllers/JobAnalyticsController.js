...
class JobAnalyticsController {
    ...
    static async getCompletedJobs(req, res) {
        try {
            ...
            console.time('DB Query: getCompletedJobs');
            // Use transaction to ensure single connection and raw queries for performance
            const completedJobs = await sequelize.transaction(async (t) => {
                    ...
                    include: [
                        {
                            ...
                            required: true,
                            attributes: [
                                'id', 'quoteNumber', 'laborTotal', 'materialTotal', 'actualMaterialCost', 'actualLaborCost'
                            ],
                            include: [
                                {
                                    model: JobAnalytics,
                                    ...
                                    required: false,
                                    attributes: [
                                        'id', 'actualMaterialCost', 'actualLaborCost', 'allocatedOverhead',
            ...
            console.timeEnd('DB Query: getCompletedJobs');
            ...
            const jobsData = completedJobs.map(job => {
                ...
                const actualLaborCost = quote.actualLaborCost ? parseFloat(quote.actualLaborCost) : null;
                const actualMaterialCost = quote.actualMaterialCost ? parseFloat(quote.actualMaterialCost) : null;
                const jobPrice = parseFloat(job.totalAmount);

                let analytics = null;
                let hasAnalytics = false;

                if (quote && quote.jobAnalytics && quote.jobAnalytics.id) {
                    ...
                    analytics = {
                        materialCost: parseFloat(ja.actualMaterialCost || 0),
                        laborCost: ja.laborSource === 'actual'
                            ? parseFloat(ja.actualLaborCost)
                            : parseFloat((jobPrice * ja.laborPercentage / 100).toFixed(2)),
                        ...
                        breakdown: {
                            materials: { amount: parseFloat(ja.actualMaterialCost || 0), percentage: parseFloat(ja.materialPercentage) },
                            labor: {
                                amount: ja.laborSource === 'actual'
                                    ? parseFloat(ja.actualLaborCost)
                                    : parseFloat((jobPrice * ja.laborPercentage / 100).toFixed(2)),
                                percentage: parseFloat(ja.laborPercentage)
                            },
                ...
                return {
                    ...
                    jobPrice: jobPrice,
                    laborCost: actualLaborCost || 0,
                    materialCost: actualMaterialCost || 0,
