...
function calculateProductionBasedPricing(params) {
  const { areas, rules, productSets, laborOnly = false } = params;
  ...
  for (const area of areas) {
    for (const item of area.items || []) {
      ...
      if (productionRate > 0) {
        ...
        // Multiply billable rate by crew size (not divide hours by crew size)
...
function validatePricingParams(params) {
  ...
  if (params.rules) {
    ...
    if (rules.billableLaborRate !== undefined && (rules.billableLaborRate <= 0 || rules.billableLaborRate > 500)) {
...
function applyMarkupsAndTax(basePricing, settings = {}) {
  ...
  const percentageFields = [
    ...
    { name: 'depositPercent', value: depositPercent, min: 0, max: 100 }
  ];
